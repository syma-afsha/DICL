...
for t in tqdm(range(total_steps), desc="Training", leave=True):
    ...
    next_obs_dict, r, terminated, truncated, info = env.step(np.array(action))
 
 

      
    penalty_reward, terminated_episode=penalty.penalty(next_obs_dict)
    if penalty_reward==-100:
        r=penalty_reward #type:ignore
        r=float(r)
    ...
    d = terminated or truncated or terminated_episode
    ep_ret += float(r)
    ep_len += 1
    ...
    if d or (ep_len == max_ep_len):
        ...
        if ep_ret<=-100:
            print("Using Dual Buffer")
         
            if dual_buffer_active:
                print("Storing episode in dual negitive buffer")
                dual_buffer.store_episode_neg(episode, t, bool(ep_success))
        # elif -70>=ep_ret>=-100:
        #     print("Using Dual Buffer")
        ...
        if dual_buffer_active:
            print("Storing episode in Positive buffer")
            dual_buffer.store_episode_pos(episode, t, bool(ep_success))
        obs_dict,_ = env.reset()
        o = process_obs(obs_dict, is_fetch_reach, config, int(act_dim))
    ...
    if (t + 1) % steps_per_epoch == 0:
        ...
        if dual_buffer_active:
            lambda_pos=config["buffer"]["dual_buffer"]["predefined"]["lambda_pos"]
            lambda_neg=config["buffer"]["dual_buffer"]["predefined"]["lambda_neg"]
