...
for t in tqdm(range(total_steps), desc="Training", leave=True):
    ...
    d = terminated or truncated or info.get("is_success", False) or terminated_episode
    ep_ret += float(r)
    ep_len += 1
    ...
    if d or (ep_len == max_ep_len):
        ...
        if ep_ret<=-100:
            ...
            if dual_buffer_active:
                ...
                current_lambda_pos, current_lambda_neg=dual_buffer.store_episode_neg(episode, t, bool(ep_success))
            ...
            if dual_buffer_active:
                ...
                current_lambda_pos, current_lambda_neg=dual_buffer.store_episode_pos(episode, t, bool(ep_success))
        obs_dict = taskenv.reset_env()
        o = process_obs(obs_dict, is_fetch_reach, config, int(act_dim))
